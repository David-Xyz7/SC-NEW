...
//RECODE BY YOGIPW AND DAPPAUHUYツ
const {
   WAConnection,
   MessageType,
   Presence,
...
//owner number
const ownerNumber = ["6285865829368@s.whatsapp.net","6285865829368@s.whatsapp.net"]   //ganti nomor lu! 
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('Sovietツ','red'),color('X','lime'),color('Botツ','red'))
})
...
client.on('message-new', async (mek) => {
	try {
		...
		if (urutan_pesan === 5000) {
			...
			const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "*DREAM BOT FREE APIKEY/nCREAT BY: DAVID", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} } }
			const isUrl = (url) => {
...
//============================CASE DASAR============================\\
                  ...
                  case 'creator':
                  case 'developer':
                  client.sendMessage(from, {displayname: "Yogiツ", vcard: vcard1}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, {displayname: "Dappaツ", vcard: vcard2}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Tuh nomer owner ku 😎 jangan spam goblog atau gw block lu',MessageType.text, { quoted: mek} )
					break  
					case 'daftar':
		...
		if (!isRegistered) return reply( ind.noregis())
				    if (isLimit(sender)) return reply(ind.limitend(pusname))
				    if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply(`*SOVIET MENU⛔*\n\n「1」Ketik ${prefix}sovietmenu 1\n*Untuk menampilkan menu image*\n\n「2」*Ketik ${prefix}sovietmenu 2*\n*Untuk menampilkan menu information*\n\nSalam URRRAAAAA!!!`)
...
// MENU COMMAND
                    ...
                    tertipu = fs.readFileSync('assets/Ara.mp3')
...
║
║Selamat ${ucapan.data.result}Kak *${pushname}*
║═════════════════❍
║🕰 *HAI   ${pushname}*
║▬▭▬▭▬▭▬▭▬▭
║⧐ ⸨ *sᴘᴀᴍ ᴀᴜᴛᴏ ʙʟᴏᴄᴋ!* ⸩
...
║▬▭▬▭▬▭▬▭▬▭
╠═════════════════❍
║⧐ ɪɴɢɪɴ sᴇᴡᴀ ʙᴏᴛ? Ketik ${prefix}sewabot
╠═════════════════════
║      ╔╦╦╦═╦╗╔═╦═╦══╦═╗
║      ║║║║╩╣╚╣═╣║║║║║╩╣
║      ╚══╩═╩═╩═╩═╩╩╩╩═╝
...
╠═══════════════════❍` 
                    client.sendMessage(from, yogi, text, { quoted: freply })
                    tertipu = fs.readFileSync('assets/onichan.mp3')
...
break
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST SOVIET BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
